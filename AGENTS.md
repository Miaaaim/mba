...
- **路由**：`HashRouter`，`/` → HomePage（单页滚动首页），`/learning` → LearningPage（课程笔记详情）。线上 URL 形如 `https://mia-portfolio.cn/mba/#/learning`
...
1. **不要改回 `BrowserRouter`**：CloudBase 静态托管（COS）没有 SPA fallback，`/mba/learning` 这类深链会直接返回 `NoSuchKey` 404。路由必须留在 hash 里。同理，站内导航一律用 `<Link>` / `navigate()` 和 `scrollIntoView`，**禁止**写 `href="#section"` 锚点——那会被 hash 路由当成路由跳转。
2. **线上空白页**：多半是静态资源用了硬编码绝对路径。检查所有 `import.meta.env.BASE_URL` 引用。
3. **部署用 `build:cloud`**：云端环境需 rebuild 原生模块（`@tailwindcss/oxide`、`esbuild`），不要用普通 `build`。
4. **课程文件命名**：必须匹配 `C{编号}_{序号}_课程名第{N}课_{笔记|记忆图}.md`，否则会被 `buildLearnIndex()` / 索引生成忽略。新增或修改课程 md 后需重新生成索引（`npm run generate:learn-index`，或直接 `npm run dev` / `build`）。
5. **`@` 别名指向根目录**：`@/src/components/...` 而非 `@/components/...`，不过当前项目统一使用相对路径。
